...
器官可以占据特定的部位（如心脏、肺、皮肤等）。占据相同部位的器官将会互斥，玩家在选择新器官时会自动吞噬旧器官以获得一定量物质。

部位系统支持三种使用方式：

  常用部位（使用枚举）
    使用 OrganPartEnum 中定义的常用部位（heart, lung, skin 等）
    这些部位在 partConfigList 中有明确的配置（显示名称、最大数量等）
    例如：part: OrganPartEnum.Heart（最多1个）

  独特部位（任意字符串）
    可以使用任意字符串作为部位标识，用于修饰和渲染气氛
    未在 partConfigList 中注册的部位默认 maxCount = 1
    例如：part: "mystical_third_eye"（神秘的第三只眼，独一无二）

  无部位限制
    不设置 part 字段（或设为 undefined）
    此类器官可以无限重复获取，不受数量限制
    例如：寄生虫、临时器官等
