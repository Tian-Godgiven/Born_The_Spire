...
type BehaviorPattern = {
    ...
    intent?: IntentType            // 意图类型（可选，声明后直接使用，不声明则从卡牌 tag 推导）
    action: {
...
`intent` 字段声明该行为模式对应的意图类型。推荐始终填写，避免从卡牌 tag 推导出错。
loop 模式下如果每轮卡牌类型不同，可以不填 intent，让系统按卡牌 tag 自动推导。
...
// result: { cards: Card[], intent?: IntentType }

// 设置意图（传入 player 用于模拟 target 端 Buff 影响）
...
1. **推荐声明 intent**：每个 pattern 和 fallback 都应填写 `intent` 字段，避免从 tag 推导出错
2. **条件是 AND 关系**：一个 `condition` 中的所有条件都要满足
3. **优先级很重要**：濒死反击等关键行为应该设置高优先级
4. **总是提供 fallback**：避免没有可用行为的情况
5. **序列模式需要维护状态**：`sequenceIndex` 会自动递增
6. **权重可以是小数**：`weights: { "card1": 1.5, "card2": 0.5 }`
7. **自定义条件很灵活**：可以实现任何复杂逻辑
8. **每回合只触发一个 pattern**：第一个满足条件的 pattern 会被使用
9. **不同回合可以使用不同模式**：通过条件控制在不同情况下使用不同的行动模式
10. **多次行动需要配置属性**：`actions-per-turn` 属性控制每回合行动次数
11. **loop 模式的 intent**：如果循环中卡牌类型不同（如攻击→强化交替），可不填 intent 让系统按卡牌 tag 推导
