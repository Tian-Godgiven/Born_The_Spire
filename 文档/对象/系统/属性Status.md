...
originalBaseValue（原始基础值）
  不受任何修饰器影响的原始值
  用于卡牌升级等场景，保留修饰器效果
  例如：卡牌伤害原始值为 5，升级后变为 8

baseValue（基础值）
  原始基础值 + 基础层修饰器的结果
  长期保留的属性值
  例如：原始值 5 + 器官提供 +3 = 基础值 8

value（当前值）
  基础值 + 当前层修饰器的结果
  可能在战斗中临时变化
  例如：基础值 8 + 战斗中 x2 = 当前值 16

### 升级场景示例

```typescript
// 初始状态
card.status.damage.originalBaseValue = 5  // 原始值
// 器官提供 +3 修饰器
card.status.damage.baseValue = 8          // 5 + 3
card.status.damage.value = 8              // 当前值

// 升级卡牌（伤害 5 → 8）
card.status.damage.setOriginalBaseValue(8)
// 修饰器自动重新计算
...
3. 写回：
   - `Status.baseValue = base`
   - `Status.value = current`

说明：
- originalBaseValue 不参与修饰器计算，只作为基础值的起点
- 修改 originalBaseValue 会触发完整的 refresh 流程
...
#### setOriginalBaseValue(status, value)
设置属性的原始基础值，并触发完整的 refresh 流程。

**用途**：
- 卡牌升级：修改卡牌属性的原始值，保留所有修饰器
- 器官升级：修改器官属性的原始值
- 任何需要"改变基础，但保留修饰器"的场景
...
card.status.damage.setOriginalBaseValue(8)
// 修饰器自动重新计算，+3 修饰器保留
// 如果之前是 5 + 3 = 8，现在变成 8 + 3 = 11
```

**实现位置**：`src/core/objects/system/status/Status.ts`
