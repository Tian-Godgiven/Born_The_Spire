...
参数：
  when: "before" | "on" | "after"  // 触发时机。on 固定在 before 之后
...
执行顺序：
  ...
  Event on 触发器
  调用 onExecute 回调
  执行 effects（每个效果：before → on → apply → after）
  Event after 触发器
  调用 onComplete 回调
...
事件取消：在 `before` 触发器中调用 `event.cancel()`，事件会被标记为已取消。

行为：
  `_applyEffects` 开头检查 `event.isCancelled()`，若已取消则整段 effects 循环跳过
  Event before 触发器已经跑完，effect before/after 与 event after 触发器**不会**再触发
  effect 执行前的取消是不生效的（进入 effect 循环后没有取消检查）

源文件：
  取消标记：`ActionEvent.ts:cancel()` / `isCancelled()`
  跳过逻辑：`transaction.ts:_applyEffects` 开头

用途：复活等"事件必须整个不发生"的场景。目前该 API 在 core 中已定义但未被生产代码使用，是预留机制。
...
意图和卡面数字不是模拟事件，不会改游戏状态。

做法
  对克隆出来的 `params` 折叠 `before` / `on` 触发器上的改参效果（力量、易伤、飞飘/飞行减半）
  入口：`previewEffect`（`src/core/objects/system/effect/previewEffect.ts`）

`ActionEvent.simulate` 仍然存在，给其它「演一遍但不落地」的路径用。意图不再走这条路。

详见 `文档/子系统/UI/效果预览系统.md`
...
事件执行是异步的
  doEvent 返回 Promise<ActionEvent>
  自己开新事务时，await 等到本事件和级联全部结算完
  已经在事务里（效果函数 / 触发器回调）时，只入队，Promise 立刻 resolve，由外层事务保证顺序
  派发之后要读结果（生命、物质等），必须 await
