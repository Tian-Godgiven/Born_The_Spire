...
`Running.vue` 先查这张 UI 表，没有再到核心 `roomRegistry` 上取该类型的 `defaultComponent`。内置房间把 Vue 写在 UI 表，避免核心反向依赖每一页；Mod 可以把组件作为 `registerCustomRoomType` 的第三参数，不必再抄一遍 UI 表。以前宝箱房只挂了核心、没写进 UI 表，而 Running 当时也不回落，所以进房会报「未找到房间类型 treasure」。
...
核心类必须注册。Vue 组件二选一：作为第三参数挂到类型上，或调用 `registerRoomComponent` 写进 UI 表。只注册类、不挂组件，进房时 Running 仍会报找不到类型。
...
不想把 Vue 交给核心注册表时，改调 `registerRoomComponent("myCustom", MyCustomRoomVue)`（`src/ui/registry/roomComponentRegistry.ts`）。地图节点图标、生成权重另见 `文档/子系统/UI/地图UI集成指南.md` 和 `文档/子系统/房间/地图配置系统使用指南.md`。
