...
```typescript
import { completeAndGoNext, openMapToLeave, finishRoomAndOpenMap, goToRoomType, completeAndGoToRoomType } from "@/core/hooks/step"

// 打开地图，不结束当前房间（水池 / 黑市 / 事件「无视他」）
openMapToLeave()

// 锁住交互并打开地图（事件成交、开完宝箱）
finishRoomAndOpenMap()

// 马上完成当前房间并打开地图（战斗胜利要立刻收掉战斗状态）
...
进房要渲染哪一页，挂在 `registerRoomType` 的第三参数 `defaultComponent` 上。`Running.vue` 按当前房间 `type` 去核心表取，不再维护第二张 UI 名单。

```typescript
roomRegistry.registerRoomType("battle", BattleRoom, Battle)
roomRegistry.registerRoomType("eliteBattle", BattleRoom, Battle)
roomRegistry.registerRoomType("treasure", TreasureRoom, TreasureRoomVue)
```

`src/ui/registry/roomComponentRegistry.ts` 只是转发：`getRoomComponent(type)` 等于 `roomRegistry.getDefaultComponent(type)`。已有类型要换皮，调 `registerRoomComponent("treasure", OtherVue)`，内部写回核心表。
...
roomRegistry.registerRoomType(
    ...
    defaultComponent: Component  // 进房 Vue 页，Running 按 type 取这一页
): void
```
...
```typescript
roomRegistry.setDefaultComponent(type: string, component: Component): void
```
...
// 按类型取进房 Vue 页
roomRegistry.getDefaultComponent(type: string): Component | undefined
...
├── initInitRooms.ts            # 开局选器官
├── initBattleRooms.ts          # 战斗（含精英 / 精英+ / Boss，共用 Battle.vue）
├── initEventRooms.ts           # 事件
├── initPoolRooms.ts            # 水池
├── initBlackStoreRooms.ts      # 黑市
├── initRoomSelectRooms.ts      # 房间选择
├── initTreasureRooms.ts        # 宝箱
├── initFloorSelectRooms.ts     # 选层
└── initDefeatVictoryRooms.ts   # 失败 / 通关
```

每个模块负责：
- 注册自己的房间类型（类 + Vue 页）
- 注册自己的房间配置（失败 / 通关没有地图配置，只登记类型）
- 导出 initXxxRooms() 函数
...
已有类型只换皮，调 `registerRoomComponent("myCustom", OtherVue)`（内部写回核心表）。地图节点图标、生成权重另见 `文档/子系统/UI/地图UI集成指南.md` 和 `文档/子系统/房间/地图配置系统使用指南.md`。
...
// 2. 注册房间类型，第三参数是进房时要渲染的 Vue 页
registerCustomRoomType("treasure", TreasureRoom, TreasureRoomVue)
...
必须先调用 registerRoomType（带进房 Vue 页），再注册该类型的配置
...
6. 进房页只认类型上的 defaultComponent

`Running.vue` 按 `currentRoom.type` 取组件。登记类型时必须带第三参数，换皮用 `setDefaultComponent` / `registerRoomComponent`。不要再往 UI 表里抄一份名单。
