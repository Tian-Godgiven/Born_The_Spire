...
战斗奖励：
  ...
  同化器官：从敌人拥有的器官中随机抽取3个（`drop: false` 的不进池），选择其中1个进行同化，也可以选择吞噬器官获得额外物质
...
战斗奖励：
  ...
  Boss器官：从Boss身上可掉落的器官中选择1个进行同化（`drop: false` 除外），与普通同化器官选项互斥
...
配置说明：
  ...
  enemyConfigs：可以是完整的 EnemyMap 配置数组，也可以是敌人 key 字符串或带覆盖的实例对象（从 enemyList 中加载后合并 behavior / statusOverrides / uiSize）
  uiSize：写在这场的实例上。`small` 缩小、`big` 放大，不写则 normal。不要按人数自动缩小
