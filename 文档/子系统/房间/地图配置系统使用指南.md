...
```typescript
{
  ...
  nodesPerLayer: {
    min: 3,  // 每层最少3个节点
    max: 7,  // 每层最多7个节点
    weights: { 3: 50, 4: 30, 5: 20 }  // 可选；不写则 min~max 均匀抽
  },
...
`roomTypes: ["treasure"]` 只改该层第一个节点，其余仍按默认规则。整层都要宝箱或水池时用 `fillType`：

```typescript
{
  layerLayouts: {
    6: { fillType: "treasure" },  // 第7层：每个节点都是宝箱
    13: { fillType: "pool" }      // Boss 门前：每个节点都是水池
  }
}
```
...
3. **房间池**
   - `generateMap` 先并入当前 Floor 的 `roomPools`，再叠传入的覆盖
   - 如果合并后仍为空，会自动从 roomRegistry 填充
   - 普通战斗带 `encounterPool: "easy"` 的进弱池，其余非 test 的进 battles
   - 本层前 3 场实际进入的普通战斗从弱池抽，精英/Boss 不计
   - 事件和 Boss 必须在 floorList 里显式列出（空事件池会灌入全部已注册 event，含 STS）
   - 默认末层是 `roomTypes: ["bossBattle"]`，从 `bossBattles` 抽，不要写不存在的 `battle_boss_default`
