...
import { lostTriggerFromSourceKey, TriggerObj  } from "../system/Trigger"
...
import { systemEntity } from "../system/ActionEvent"
type Stack = {
...
export type StateData = {
    ...
    effect?:(getter:Target,state:State)=>TriggerObj//状态对应的触发器
    behavior?:Record<string,((getter:Target,state:State)=>void)>//状态自身的行为触发器，会为对象添加对应的触发器但作用与state对象上
}
...
export class State{
    ...
    public effect?:(getter:Target,state:State)=>TriggerObj
    public behavior?:Record<string,(getter:Target,state:State)=>void>
...
function getState(source:Entity,medium:Entity,target:Target,state:State){
    ...
    function getNewState(){
        doBehavior("getState",source,medium,target,{state},()=>{
            //使得目标获得触发器
            const getTrigger = state.effect
            if(getTrigger){
                const trigerObj = getTrigger(target,state)
                target.getTrigger(trigerObj)
            }
            //使得目标获得状态行为触发器
            const behavior = state.behavior
            if(behavior){
                for(let [key,func] of Object.entries(behavior)){
        ...
        switch(state.repeate){
            ...
            case "stack":
                ...
                for(let stackKey in state.stacks){
                    ...
                    //改变旧对象的层数
                    changeStateStack(newValue,target,)
...
function lostState(source:Entity,medium:Entity,target:Target,stateKey:string){
    doBehavior("lostState",source,medium,target,{stateKey},()=>{
        //目标会失去stateKey为来源key的所有触发器
        lostTriggerFromSourceKey(target,stateKey);
        //目标失去该状态
        const index = target.state.findIndex(state=>state.key == stateKey)
...
export function changeStateStack(newValue:number,target:Target,stateKey:string,stackKey:string="default"){
    ...
    if(state){
        ...
        //目标失去该状态,因层数下降导致的失去状态的来源和媒介都会是系统对象
            ...
            lostState(systemEntity,systemEntity,target,stateKey)
        }
    }
